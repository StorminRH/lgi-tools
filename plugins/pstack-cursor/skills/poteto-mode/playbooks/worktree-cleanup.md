...
LGI adoption: audit and report only. Do not execute deletion or reset steps below. The bundled audit has macOS and worktree-classification limitations; its safe category is not evidence of disposability.
...
5. Prune the confirmed set. Per path, `[disabled in LGI adoption: forced worktree removal] <path>`. If the dir survives on ignored build artifacts, `rm -rf` it, then `git worktree prune`. Branch refs survive, so no commits are lost. Confirm with `df -h /` and re-list.
