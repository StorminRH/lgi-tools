...
> = {
  ...
  'eve-sso': {
    wrapper: { module: 'src/platform/auth/eve-sso.ts', symbol: 'exchangeCodeForToken' },
    timeout: '10s per request (OUTBOUND_FETCH_TIMEOUT_MS via fetchWithTimeout).',
    retryableErrors:
  ...
  convex: {
    ...
    rateLimit: 'None; Convex is first-party infrastructure with no shared budget to protect.',
    idempotency:
    ...
    telemetryFields: "'cron_sync_sweeper' (the watchdog that detects a lagging Convex scan).",
  },
  'upstash-redis': {
  ...
  'vercel-platform': {
    noProgrammaticSurface: true,
    fact: 'The app makes no Vercel API call. Its only src artifact is the build-injected @vercel/speed-insights beacon component in src/app/layout.tsx, which reports from the browser and exposes no app-controlled request. Deployment and environment actions are operator CLI work, outside the request path.',
  },
  'github-tooling': {
    wrapper: { module: '.agent-local/github_api.py', symbol: 'request' },
    timeout:
  ...
  fuzzwork: {
    ...
    retryableErrors: 'None.',
    backoff: 'None.',
    rateLimit: 'No app-side limiter; this is the low-volume fallback source, not the primary.',
  ...
  'ccp-static-data': {
    wrapper: { module: 'src/data/eve-data/source.ts', symbol: 'downloadSdeJsonl' },
    timeout:
  ...
  'ccp-image-cdn': {
    ...
    backoff: 'None.',
    rateLimit: "CCP's image server limits; requests go direct and never through the Vercel optimizer.",
  ...
  'anoik-statics': {
    ...
    retryableErrors:
      'None. An unreachable, timed-out, or non-200 feed resolves to unavailable rather than retrying.',
    backoff: 'None.',
    rateLimit:
      'Weekly cron plus operator-triggered on-demand only; the unchanged path is a conditional GET that transfers zero body bytes.',
    idempotency:
      'Read-only conditional GET. A repeat with the same ETag is safe and normally returns 304.',
    degradation:
      'A failed refresh records feed-unavailable and leaves the promoted Neon copy untouched; serving never calls anoik.is.',
