...
const ROUTE_ENTRIES: readonly IdempotencyEntry[] = [
  ...
  mutationRoute('src/app/api/admin/wh-statics/route.ts', 'key-protected', 'Refresh uses the shared advisory lock and atomically supersedes any prior pending snapshot; promote and reject accept only a pending snapshot, so a repeated review action is refused without changing the promoted copy.'),
