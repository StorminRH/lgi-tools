...
import {
  ...
  type UserCorpAccess,
} from '@/platform/auth/corp-access';
import {
...
export async function getCorpStructuresForUserOnView(userId: string): Promise<ViewerCorpStructuresResult> {
  ...
  const corporationIds = [...access.memberCorpIds];
...
export async function getCorpStructuresPageData(userId: string): Promise<CorpStructurePageView[]> {
  ...
  const smFlags = await Promise.all(
    corporationIds.map(
      async (corporationId) =>
        [corporationId, await userHoldsCorpRoleFromAccess(access, corporationId, CORP_STRUCTURES_REQUIRED_ROLES)] as const,
    ),
  );
...
/**
 * Whether the access snapshot holds one of `requiredRoles` in the corp via ANY of
 * its fresh member pilots — the Station_Manager gate on the sharing +
 * rig-completion mutations. Composes the auth membership set with the ESI roles
 * read (the same vend + readRoles the sync engine's Director resolution uses), so it
 * belongs here in the composition layer, not in either feature slice. Returns true
 * on the FIRST in-corp pilot that holds the role; a pilot whose token can't be
 * vended or whose roles can't be read simply doesn't contribute.
 */
async function userHoldsCorpRoleFromAccess(
  access: UserCorpAccess,
  corporationId: number,
  requiredRoles: readonly string[],
): Promise<boolean> {
  for (const characterId of memberCharacterIdsForCorp(access, corporationId)) {
...
export async function stationManagerGate(
  ...
  let decision;
  try {
    decision = await authorizeCorpMutation(access, corporationId);
  } catch {
    return {
      ok: false,
  ...
  if (!(await userHoldsCorpRoleFromAccess(access, corporationId, CORP_STRUCTURES_REQUIRED_ROLES))) {
    return {
      ok: false,
