...
// The common single-fetch → verdict mapping for slices WITHOUT a bespoke plan fn
// (owned blueprints / assets — the others already ship a tested planXPersist that is
// itself a PersistVerdict). unchanged → stamp; error → skip (unless mapError
// overrides, e.g. corp jobs' 403 → needs_role); fresh → onFresh, where a null
// projection (contract mismatch) becomes skip and otherwise saves the payload.
