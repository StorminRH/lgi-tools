...
test('groups fresh members, drops stale and null corps, treats the TTL boundary as fresh, and freezes the snapshot', () => {
  ...
  const frozen = createCorpAccessSnapshot('u1', [row(101, 2000)], false, NOW.getTime());
  expect(Object.isFrozen(frozen)).toBe(true);
  expect(Object.isFrozen(frozen.allCharacterIds)).toBe(true);
  expect(Object.isFrozen(frozen.corporationIds)).toBe(true);
  expect(Object.isFrozen(frozen.characterIdsByCorporation)).toBe(true);
  expect(Object.isFrozen(frozen.characterIdsByCorporation[2000])).toBe(true);
});
