...
function breakdownLines(score: MarketScore, history: MarketHistoryInputs | null): string[] {
  const lines: string[] = [];
  const lq = score.liquidity;

  if (lq.timeToClearDays === null) {
    lines.push('Time to clear: unknown — no demand history yet.');
  } else {
    const wall =
    ...
    if (lq.instantDumpUnits !== null) {
      lines.push(`Dump into buy orders now and ≈${formatQuantity(lq.instantDumpUnits)} units sell instantly.`);
    }
  }
  ...
  const breakdown = (
    <div className="flex flex-col gap-1.5 max-w-[320px]">
      ...
      <ul className="flex flex-col gap-1 text-[11px] text-text leading-snug">
        {breakdownLines(marketScore, history).map((line) => (
          <li key={line}>{line}</li>
        ))}
      </ul>
      {contextLine(history) && (
        <div className="text-[10px] text-muted">{contextLine(history)}</div>
      )}
