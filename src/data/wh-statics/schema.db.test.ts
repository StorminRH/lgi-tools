...
describe.skipIf(!harness.reachable)('wh-statics schema (real Postgres)', () => {
  it('reflects both tables with their columns, keys, and status enum', async () => {
    ...
    // LIKE INCLUDING ALL keeps the column typed against the public enum.
    const statusLabels = await harness.sql<{ enumlabel: string }[]>`
      SELECT e.enumlabel
      FROM pg_type t
    ...
    `;
    expect(statusLabels.map((row) => row.enumlabel)).toEqual([
      'pending',
      'promoted',
      'rejected',
      'superseded',
  ...
  it('accepts a pending snapshot row and a promoted statics assignment', async () => {
    ...
    const rows = await harness.db.select().from(whSystemStatics);
    expect(rows).toEqual([
      {
        systemId: 31000001,
        code: 'E175',
