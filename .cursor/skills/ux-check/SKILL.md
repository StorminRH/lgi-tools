...
console, page errors, network). Write sanitized diagnostic artifacts under
...
suite and the standing GitHub Actions pipeline.
...
definitions for changed interactions; (4) for auth probes — local fixture-owned
sessions or operator-exported `E2E_STORAGE_STATE` / `UX_STORAGE_STATE` with
`E2E_SKIP_SEED=1` for deployed reads. The migrated Playwright fixtures own local
seeding and cleanup; deployed inputs are documented in `docs/ux-check/README.md`.
...
logs and failure diagnostics under `docs/ux-check/captures/` only to
...
Anonymous sweeps verify signed-out gates. Signed-in local runs use the
Playwright fixture-owned sessions and cleanup.
...
with `PLAYWRIGHT_BASE_URL`, and use remote authentication per the remote-probes section.
...
The selected Playwright lane owns its Next server. Prepare the local backends
and production build it requires; Cloud Agent caveats live in
...
```bash
pnpm ux-check --grep 'route-home|route-atlas'
E2E_SCENARIOS=atlas-access pnpm test:e2e:local
```

Viewports and authentication come from the selected journey registry. Set
`PLAYWRIGHT_BASE_URL` and, for deployed reads, the expected principal inputs in
`docs/ux-check/README.md`. Set `VERCEL_AUTOMATION_BYPASS_SECRET` for protected
Vercel preview/production URLs. Evidence: gitignored
`docs/ux-check/captures/e2e-report.json` and `sanitized-failures/*.json`.
...
`docs/ux-check/captures/e2e-report.json`. Fails on assertion failure, probe
...
1. Read `docs/ux-check/captures/e2e-report.json` for each selected run.
