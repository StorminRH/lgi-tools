...
description: Use before merging development onto staging, or staging onto main. Independent structure and behavior seats, then thermos and no-comments, on that frozen merge.
---

# Adversarial review

One frozen merge. Independent seats. Then `thermos` and `no-comments`.
...
other seat when the operator names it. Brief each seat with frozen
...
Launch those seats by type name. Omit Task `model` so the agent file pin
applies.
