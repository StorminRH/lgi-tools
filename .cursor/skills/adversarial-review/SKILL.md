...
description: Use on an Origin draft and before every merge onto staging or main. Independent structure and behavior agents, then thermos and comment-sicko, on a freeze. Reviewers run origin pr diff.
---

# Adversarial review

One freeze. Independent agents. Then `thermos` and `comment-sicko`.
Every seat returns before the caller batches. One review round.
...
This skill is the review gate on an Origin draft, including every
merge onto `staging` or `main`. Plans and Ordered work use
`structure-reviewer` and `behavior-reviewer` from those skills.

## 1. Freeze

Done when the Origin change has a frozen identity, the tree is
still, and every change group has authority.

The subject is that Origin change. Brief every seat with the
...
did not. Keep the tree still until every selected seat has
returned.
...
Launch `structure-reviewer`, `behavior-reviewer`, `thermos`, and
`comment-sicko` in the same turn. Add the other agent when the
operator names it. Brief each seat with the change number,
authority, and operator emphasis. Each seat gets a fresh brief.
The brief is the change number.
...
## 3. Thermos and comment-sicko

Done when `thermos` has returned its synthesized verdict and
`comment-sicko` has returned its report.

They run on the same freeze. Read and follow
`.cursor/skills/thermos/SKILL.md`. `comment-sicko` reports;
`no-comments` inspects that report during this freeze and stops
at the accepted list. The caller batches deletions and code
fixes after every seat has returned.

Treat accepted findings from `thermos` and `comment-sicko` as
this review's findings.

## 4. Verify

Done when every reported failure from the agents, `thermos`, and
`comment-sicko` is reproduced or disproved, and each root cause
is accepted or rejected once as `BLOCKER`, `MAJOR`, or `MINOR`.
The tree is still the freeze head.
...
`PASS` when no accepted finding remains and `comment-sicko` has
reported. `CORRECTIONS_REQUIRED` when the caller has an accepted
list to batch. `PASS` is the review gate for the named merge
when the list is empty. It does not merge.
