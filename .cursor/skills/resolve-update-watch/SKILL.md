...
description: Resolve an open update-watch digest by applying safe dependency fixes, recording deferred and informational findings, and preparing a reviewed unmerged pull request. Use when the operator asks to address or resolve an Update watch issue.
---

# Resolve an update-watch digest

Companion to report-only `update-watch`. That skill files the digest issue; this
one resolves it — apply safe fixes, record the rest in the update-watch
baseline, and take a PR to review without merging.
...
2. Read the digest: **Security advisories**, **Major versions**, per-source
   **Service/source changes**, and fenced `update-watch-deltas` keys.
