...
- **Context7 MCP** — current library docs. **Use it during planning** (plan mode) to confirm current API shapes and conventions before proposing an approach — especially for Next.js, Drizzle, and anything version-sensitive, since training data lags this codebase's versions. The cost of a stale assumption here is a whole session built on the wrong API (the SRI/CSP and Cache Components work both turned on details that only the current docs got right). When a plan depends on how a library *currently* behaves, verify it here first rather than asserting from memory; cite what you found in the plan.
