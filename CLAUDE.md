...
- `src/lib/` — cross-cutting helpers importable from anywhere (unclassified by the boundary rules; the `@/lib/array`-from-a-data-slice precedent). Home of the two invariant-backed modules from 3.4.T — the typed env accessor (`env.ts`) and the typed fetch client (`api-client.ts`) — alongside `rate-limit.ts` and `alerts.ts`.
