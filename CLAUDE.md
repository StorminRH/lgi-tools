...
- Use `.claude/skills/` for Claude-specific workflow mechanics; shared policy remains in the canonical guides and workflow documents.
- Invoke project skills with Claude's native `/skill-name` syntax, such as
  `/start-session`. Edits inside the existing `.claude/skills/` tree reload in
