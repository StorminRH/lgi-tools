...
- **Minimize per-request work; move toward static where the CSP allows.** Under the *strict* nonce CSP (the policy in force through 3.0.4.5), App Router pages can't be served as `○` static — every page embeds inline RSC flight-data scripts that hydration needs, and a baked static page has no per-request nonce to admit them (SRI only signs *external* scripts; `'unsafe-inline'` is forbidden). That negative result is recorded in `docs/VERSION_3.0.4.3_CSP_DECISION.md`. **The conversion track (3.0.4.6–3.0.4.9, see `docs/VERSION_3.0_PLAN.md`) is the sanctioned plan to change this**: relax the CSP to a basic `script-src 'self'`, clear the cookie/DB render-time blockers, then reclaim static route-by-route. Relaxing the CSP toward `'self'` as part of that track is *endorsed*, not a violation of this principle — do not raise it as a conflict. Until 3.0.4.9 lands and verifies the reclaim, treat routes as dynamic (`ƒ`) and make each dynamic render cheap regardless: cache rarely-changing reads (the stable `'use cache'` equivalent, not deprecated `unstable_cache`), batch DB queries (no N+1), isolate and defer per-request data, and avoid needless compute. A page being interactive is fine; a page doing redundant per-request DB work is not. (3.0.4.9 reconciles this principle toward "static by default" once the reclaim is proven.)
