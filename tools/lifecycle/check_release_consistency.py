...
(the triplet names the latest terminal row). A release PR is authored
directly in the ``reconciled`` state — it already carries the delivered
identity and matching APP_VERSION — so main is truthful the
moment it merges, with no separate post-merge reconciliation. The checker is
read-only. Land and promote leave APP_VERSION and the roadmap untouched,
so the resting state stays ``reconciled`` until the release cut.
"""
