...
- Gitignored private docs, agent configuration, and local utilities are shared branch-independent workspace state. At close-out, audit every ignored path touched during the session: keep only deliberate durable policy or memory, reconcile both runtime adapters, remove temporary artifacts, and run the drift gate after policy changes. A clean `git status` does not prove this local state is reconciled.
