...
directions, then acts only on the one the user picks. The point is to **respond
...
- The cited files/lines exist and say what's claimed (`git grep -n`, Read).
...
- Whether the same root cause appears **elsewhere** — one dangling reference,
  ...
  run a quick read-only sweep to size it (a `Workflow` of read-only `Explore`
...
a plain-English **recommendation**. Then present the response directions with
`AskUserQuestion` (recommended option first, labeled "(Recommended)"). Pick the
...
with a contributor, how wide to go), that's an `AskUserQuestion` — not a default
you silently choose.
...
- Ship via the **`close-out` skill** (push → PR → Greptile loop → squash-merge),
  which carries the merge authorization and the green-only gate. A non-user-facing
  change skips the changelog/version bump and the UX review pause; a user-facing
  one does not.
