---
name: how
description: "Use when invoked to explain how code works, walk through runtime behavior, identify ownership, or critique architecture. Use why for design rationale and history."
---
...
Spawn explorers in parallel within the available agent slots:

- `agent_type`: `default`
- `model`: your configured how-explorer model (default `gpt-5.6-sol`, `reasoning_effort: "xhigh"`)
...
After the explanation is complete, spawn one architectural critic per entry in your configured how-critics list (defaults `gpt-5.6-sol` with high reasoning and `gpt-5.6-sol` with xhigh reasoning; keep both independent seats), in parallel within the available agent slots.
...
Use lead judgment to assess the evidence and scope of each finding.
