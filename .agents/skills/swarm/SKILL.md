...
4. Pick the worker model from `swarm workers` in `.agents/MODELS.json` as the required source of model choices. For a model race, name each arm's model up front.
