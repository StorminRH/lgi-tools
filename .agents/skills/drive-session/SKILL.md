...
description: >-
  ...
  out exploration or mechanical sub-tasks to GPT workers, when deciding which
  model handles a lifecycle stage, or when this runtime is itself the delegate
  and must honor the stop boundary. Complements start-session; never replaces
  its resolver directive, verification gates, or close-out.
---
...
Two seats exist for this runtime. As the **orchestrator**, apply the routing
table and procedure below. As the **delegate** (a prompt names the approved
plan and an orchestrator), execute only the named plan steps, honor the stop
boundary, and use the question protocol instead of guessing.

## Routing table

| Stage | Model / effort | Mode |
| --- | --- | --- |
| Planning, orchestration, acceptance, commits, PR, close-out | Orchestrating session | native |
