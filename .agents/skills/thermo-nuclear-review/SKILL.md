...
Audit the caller's GitHub change with `gh pr diff <N>` and read the
...
the GitHub change discussion with `gh pr view --comments` and `gh api`
review-thread queries. Use `--help` for supported read arguments. GitHub
is the primary review record.
