...
Read substantive PRs on their owning forge. GitHub is the land forge:
use `gh pr view <number>` and read its review threads. Consult the
installed CLI help for additional read commands. Historical Origin PRs
retain their original identity and preserved discussion. Linear owns tickets.
...
Before spawning investigators, discover the tools exposed in the Codex session and any installed CLIs that can read the same sources. Use tool search when available. Record missing capabilities; do not install plugins as part of this investigation.
...
Launch matching investigators in parallel within the available agent slots; continue in batches until all categories are covered. One investigator per category lets each specialize in one tool's query vocabulary and result shape. Keep each investigator within one evidence category.

Subagent config (each):

- `agent_type`: `default`
- `model`: your configured why-investigators model (default `gpt-5.6-sol`, `reasoning_effort: "xhigh"`)
- Brief: read-only evidence collection. Use available connectors and CLIs without changing permissions, files, or external state. Record inaccessible sources as gaps.
...
1. **Source control investigator**. Local git history, GitHub PRs, preserved Origin or historical PRs, code comments, and tests. Always spawn; record unavailable remote history as a gap. Best at surfacing *implementation-time rationale captured during review*. PR descriptions stating the problem, review threads debating alternatives, inline comments encoding non-obvious constraints, test names that encode motivating edge cases, and commit messages linking tickets or incidents. Most trustworthy because it ties directly to the diff that shipped.
...
- `agent_type`: `default`
- `model`: your configured why-synthesizer model (default `gpt-5.6-sol`, `reasoning_effort: "high"`)
- Brief: read-only synthesis and citation checks using available connectors and CLIs; no outward writes.
