...
description: >-
  ...
  is up, run the fast headless Playwright capture (`pnpm ux-check <routes…>`), and
  report the console/network findings + screenshots inline. This replaces the
...
- A changed feature/UI component → the route(s) that render it (you know which from
  the session — e.g. a wormhole-sites component → `/sites` and a `/sites/<id>`).
...
- **UP** → use it as-is.
- **DOWN** → start it in the background with the loopback host (bypasses the macOS
  firewall re-prompt): `pnpm dev` — i.e. `next dev -H 127.0.0.1`. The capture
  script polls for readiness, so you can start it and run the sweep right after.
  (Logged-out capture needs only Next; you don't need Convex/Docker for it.)
