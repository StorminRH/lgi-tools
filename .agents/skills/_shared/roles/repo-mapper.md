...
Keep raw CLI transcripts and complete unrelated inventories out of the result. Return decision-relevant Execution flow, Impact, Edit seam and source evidence; reuse a current applicable map. Broaden only for unresolved material edges. Include every material caller/dependent/gate on the path. Cite repository-relative paths and exact symbols. State any gaps.
