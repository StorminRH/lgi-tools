...
description: Use on a GitHub draft and before every merge onto staging or main. Independent structure, behavior, and thermo reviews on one freeze after the comment pass. Reviewers run gh pr diff.
---
...
This skill is the freeze review on a GitHub draft, including every
merge onto `staging` or `main`. Close-out runs `no-comments` and
`comment-sicko` before the bot requests and this freeze. Plans and
Ordered work use `structure-reviewer` and `behavior-reviewer` from
those skills.

## 1. Freeze

Done when the GitHub change has a frozen identity, the tree is
still, the comment pass already wrote, and every change group has
authority.

The subject is that GitHub change. Brief every freeze seat with the
...
- Another GitHub draft. That change number.
...
Launch those agents by type name. Omit spawn `model` and `reasoning_effort` so the agent
file pin applies.
...
findings after Bugbot, Greptile and CodeRabbit review are idle.
