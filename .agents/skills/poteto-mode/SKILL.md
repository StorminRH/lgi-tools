...
- Any prose surface → the **unslop** skill. Your reply is a prose surface. Write it per **Writing the reply**. Agent-facing prose also follows the native **skill-creator** skill for authoring SKILL.md files.
- Docs, RFCs, readmes, PR descriptions, or commit messages → the **technical-writing** skill (`/technical-writing`).
- Before commit → the `deslop` skill bundled from `cursor-team-kit` (`/deslop`).
...
- Broken skill mid-task → report the concrete gap and propose a scoped repair. Do not silently broaden the current work.
...
**Proceed within the agreed scope.** Apply the LGI workflow's authorization and design checkpoints; available tools do not authorize external actions.

**Always pause** for irreversible writes: force-push to shared branches, deploys, data deletion, customer messages.

**Session scope:** Autonomous execution requires an explicit request and does not waive the LGI workflow's merge, deployment, or messaging boundaries.
...
**Defaults for every `Task` call.** `run_in_background: true`, agent mode (respect the review scope and native sandbox), file pointers not inlined context, explicit model per role (configurable via `/setup-pstack`. Defaults `gpt-5.6-sol` for code, `gpt-6-astra` for prose and judgment). Code delegates tier by difficulty. The hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment model (`gpt-6-astra`), whether the task needs judgment on vague intent or is a precisely specified sequence of steps to execute to the letter. Trivial mechanical edits go to your fast code model. Per-role entries in this edition's `MODELS.json` override these defaults and the model choices in the routed skills (`how`, `why`, `arena`, `swarm`, `architect`, `interrogate`, `reflect`). A role without a configured entry must be resolved before spawning, and a role line of `inherit-parent` or `auto` runs that role on the parent chat model (omit Task `model`).
...
- **Shipping.** The half after Babysit. Independently verifying a green stack, then landing the contiguous verified run bottom-up through GitHub. `playbooks/shipping.md`.
