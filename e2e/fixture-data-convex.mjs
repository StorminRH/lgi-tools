...
import { requireLocalConvexEnvironment } from './fixture-data-local.mjs';

const execFileAsync = promisify(execFile);

export function requireSupportedConvexCleanup() {
  const installed = createRequire(import.meta.url)('convex/package.json');
...
export async function localConvexCommand(args) {
...
export async function localConvexRun(functionName, args) {
...
export async function censusConvexFixtures({ mapIds, userIds }) {
  const source = `
...
export async function purgeConvexFixtures({ mapIds, userIds }, run = localConvexRun) {
  const errors = [];
...
export function ownedSyncDocuments({ rows, userIds }) {
...
export async function removeOwnedSyncRows(scope, run = localConvexRun) {
