...
| `feedback/route.ts` ↔ `preferences/route.ts` | **Temporary debt.** Low-value `safeParse` → 400-on-bad-body scaffolding (this was `contact ↔ feedback` before the contact form went static in 3.6.25); a shared `parseJsonBody`/`badRequest` helper is a future tidy. |
