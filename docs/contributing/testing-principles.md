...
| Playwright log-driven UX / tiny E2E (`pnpm ux-check`, `pnpm test:e2e`) | Changed user-facing routes during Ordered-work / pre-close-out UI gates, plus a very small number of authenticated happy-path smokes. See [end-to-end testing](./end-to-end-testing.md). Route sweeps and operator pause are the `ux-check` skill; durable probes live in `docs/ux-check/README.md`. | Visual approval, layout feel, edge cases, or anything Vitest can falsify. Not part of `pnpm verify`. |
