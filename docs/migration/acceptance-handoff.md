...
Notes for [LGI-117](https://linear.app/lgitools/issue/LGI-117). Do not retire
Origin, cancel Depot, or write to Linear from this file. Written 10 September
2026.
...
| GitHub landing steps | CONTRIBUTING. Prep PRs [#504](https://github.com/StorminRH/lgi-tools/pull/504) through [#508](https://github.com/StorminRH/lgi-tools/pull/508). |
...
zero pending. That replaces the Neon-branch CI design. Do not add a second
Neon CI path.
...
Replaced by GitHub delivery and manual-only Verify:
...
The `origin pr diff` skill rewrite is in PR
...
inventory PR lands. Until then, the known title pairs are Origin 109→GitHub
471, 133→478, 135→479, and 137→481. Origin #21 from LGI-46 has no GitHub
title in the 10 September counts.

## Open gaps

- Ryan still configures GitHub required checks and the rest of the cutover
  (LGI-112 remainder).
...
- Vercel git reconnect and live staging or production proof stay with Ryan
  (LGI-116).
- Sync Manager 3:30am ET Origin-to-GitHub write should be turned off. The
...
PR 496 landed verify/build/e2e. Prep PRs 504-508 are open.

Preserved history: Origin 109/133/135/137 map to GitHub 471/478/479/481.
The inventory lists current refs. No ref was moved.

Replaced: LGI-23 Neon CI branches. LGI-46 Origin review/comment and Depot
