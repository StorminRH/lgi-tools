...
| PL-003 | Loading presentation | Content-shaped holes often collapsed to a word. | Route matrix and fallback inventory recorded below. | Loading primitives and route pages | Implemented; pending operator review |
...
| PL-005 | Route shells | Audit every classified route for its richest honest shell. | Every route in `route-classification.json` is recorded below. | Route owners | Implemented; pending operator review |
| PL-006 | House style | Static-shell and fallback composition rules were incomplete. | Installed Cache Components guide plus current route audit. | `src/AGENTS.md`, loading primitives | Implemented; pending operator review |
...
| PL-010 | Industry jobs imagery | Science corp jobs showed monograms. | Live rows with activities 3/4/5 repeated the blueprint as `product_type_id`; `/icon` returned 400 while `/bp` returned 200. Resolver is now activity-aware. | `type-images.ts` | Implemented; authenticated visual review pending |
