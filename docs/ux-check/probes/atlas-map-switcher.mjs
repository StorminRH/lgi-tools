...
function ensureSecondMap(_page, firstMapId) {
  const second = process.env.UX_SECOND_MAP_ID;
  if (!second || second === firstMapId) throw new Error('BLOCKED: second run-owned map missing');
  return second;
}
...
export default {
  ...
  async run({ page, check }) {
    ...
    if (!firstMapId) throw new Error(`BLOCKED: required run-owned fixture unavailable`);
