...
| EVE type-image intent resolver | `src/data/eve-data/type-images.ts` | Choose which rendition (item icon, 3D render, blueprint) each surface requests through per-intent resolvers instead of call-site variant literals. | Image-variant selector family in `eslint.config.mjs` with resolver-owner exemption, plus `scripts/image-variant-rail.test.mjs` |
...
| Stage-specific lifecycle skills | `.agents/skills/`, `.claude/skills/` | Keep planning, execution, review, close-out, and audit stages separate while returning control to the resolver. | Paired-skill and reconciliation-ledger checks in `.agent-local/check_agent_drift.py` |
| Shared policy manifest and drift gate | `.agent-local/policy-manifest.json`, `.agent-local/check_agent_drift.py` | Derive the cross-runtime policy surface, skill-reconciliation ledger, paired skills, hooks, probes, and mandatory checker wiring from one manifest. | `.agent-local/test_agent_drift.py` |
...
| PL-014 | UI/design | create | EVE type-image rendition choice was duplicated and drifted per call site: an inline hero ternary, a node-view `'icon'` default, planner-private `nodeIcon`/`isRenderableCategory` helpers, and `/industry` landing rows that showed monogram initials instead of images. 3.9.3.2 created the shared per-intent resolver, migrated every call site, and railed variant literals. | Every EVE type-image call site states intent (`itemImage`/`heroImage`/`nodeImage`/`jobImage`) and receives a resolved descriptor from `src/data/eve-data/type-images.ts`; rendition literals outside the resolver are lint-banned. | M | Delivered |
